...
void timeSort(){
	...
	TFile * f1 = new TFile("../../caen/data/compass_firstdata_4.root", "READ");
	...
	for(int i = 0 ; i < nEntries; i++){
      ...
      for( int k = 0; k < 100; k++){
		...
		for( int j = i+1 ; j < i + 10; j++){
			...
			dTime = ((int)Timestamp - (int)ezero_t[0])/1e8;
			
			//printf("     (%3d,%3d) ( %15f, %15f )= dTime : %f \n",i,j, Timestamp/1e8, ezero_t[0]/1e8, dTime);
			
			if( Channel == 1 && TMath::Abs(dTime) < 0.001) {
				countCh ++;
				ch[1] = 1;
				ezero_t[1] = Timestamp;
				ezero[1] = (Float_t)Energy;
				//printf("      4  %d | %d, %15llu\n", i, ch[1], ezero_t[1]);
				break;
			}	
		}
